function renderProvince(slug) {
  ...
  setTimeout(function() {
    ...
    /* Quick info grid */
    function qCard(icon, label, val) {
      return '<div class="quick-info-card">'+
        '<div class="quick-info-icon">'+icon+'</div>'+
        '<div><h3>'+label+'</h3><p>'+val+'</p></div>'+
      '</div>';
    }
    var iHome   = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>';
    var iPeople = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>';
    var iArea   = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 3H3v18h18V3z"/><path d="M3 9h18M3 15h18M9 3v18M15 3v18"/></svg>';
    var iLang   = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>';
    var quickInfo = '<div class="quick-info-grid">'+
      qCard(iHome,'Ibu Kota',province.capital)+
      qCard(iPeople,'Populasi',province.population)+
      qCard(iArea,'Luas Wilayah',province.area)+
      qCard(iLang,'Bahasa Daerah',province.language)+
    '</div>';
      ...
      var cards = items.map(function(item) {
        return '<div class="media-card">'+
          '<div class="media-img" style="background-image:url(\''+getImg(item)+'\');background-color:var(--bg3)">'+
            '<div class="media-overlay"><h3>'+getTitle(item)+'</h3><p>'+getDesc(item)+'</p></div>'+
          '</div>'+
        '</div>';
      }).join('');
    ...
    if (province.facts && province.facts.length) {
      factsHTML = '<div class="detail-section reveal">'+
        ...
        province.facts.map(function(f,idx) {
          return '<div class="fact-card">'+
            '<div class="fact-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg></div>'+
    ...
    var ctaSection = '<div class="detail-cta-section reveal">'+
      ...
      '<p style="font-size:13px;color:var(--text3);margin-bottom:20px">'+
        ...
        ' &nbsp;·&nbsp; Reward: <span style="color:var(--gold);font-weight:700">'+diff.keyReward+' kunci</span>'+
      '</p>'+
    ...
    var heroBg = province.heroImage
      ? heroOverlay + ',url(\'' + province.heroImage + '\')'
      : heroOverlay;

    setPage(
      navbarHTML('')+
      '<div>'+
        '<section class="detail-hero reveal" style="background-image:'+heroBg+';background-size:cover;background-position:center">'+
